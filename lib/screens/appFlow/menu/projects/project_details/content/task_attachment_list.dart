...
import 'package:hrm_app/screens/appFlow/menu/projects/widgets/attachment_widget.dart';
    ...
    return NoInternetScreen(
      ...
      child: Consumer<CrmTaskDetailsProvider>(builder: (context, provider, _) {
        return Scaffold(
            ...
            body: provider.crmTaskDetailsResponse?.data?.taskDetails?.files != null
                ? ListView.builder(
                    padding:
                        EdgeInsets.symmetric(vertical: 20.h, horizontal: 14.w),
                    shrinkWrap: true,
                    itemCount:
                        provider.crmTaskDetailsResponse?.data?.taskDetails?.files?.length,
                    itemBuilder: (BuildContext context, int index) {
                      final data =
                          provider.crmTaskDetailsResponse?.data?.taskDetails?.files?[index];
                      Files file = Files(
                          id: data?.id,
