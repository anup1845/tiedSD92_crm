...
class FeedbackDiscussionItem extends StatefulWidget {
  ...
  const FeedbackDiscussionItem({Key? key, this.discussion, required this.projectId}) : super(key: key);
...
class _FeedbackDiscussionItemState extends State<FeedbackDiscussionItem> {
  ...
  Future commentDelete(id) async{
    final response = await CrmTaskRepository.feedbackCommentDelete(commentId: id);
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                  ...
                  image: DecorationImage(image: NetworkImage('${widget.discussion?.avatar}'))
              ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text('${widget.discussion?.createdBy}',
                    style: TextStyle(
                        color: Colors.black,
                  ...
                  Row(
                    children: [
                      ...
                      Icon(
                        Icons.thumb_up,
                        size: 12.sp,
                        color: widget.discussion!.liked! ? Colors.blue : Colors.grey,
                      ),
                       ...
                       InkWell(
                         onTap: (){
                           ...
                           child: Icon(Icons.edit_outlined,  size: 12.sp,color : Colors.grey)),
                            const SizedBox(width: 4.0,),
                      InkWell(
                          onTap: ()async{
                            final response = await commentDelete(widget.discussion!.id);
                            if(response['result'] == true){
                             setState(() {
                               CrmClientRepository.getProjectDetailsData(id: widget.projectId);
                               print('click');
                             });
                            }
